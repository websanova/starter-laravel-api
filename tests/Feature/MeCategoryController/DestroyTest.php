...
uses()->group('me.category.destroy');
...
test('user can delete their category', function () {
    ...
    $response = $this->actingAs($user)->deleteJson("/me/categories/{$category->id}");
...
test('deleting a category nullifies bookmarks', function () {
    ...
    $this->actingAs($user)->deleteJson("/me/categories/{$category->id}");
...
test('unauthenticated user cannot delete a category', function () {
    ...
    $response = $this->deleteJson("/me/categories/{$category->id}");
