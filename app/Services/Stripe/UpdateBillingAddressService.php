...
class UpdateBillingAddressService implements UpdateBillingAddressProvider
{
     ...
     * that the provider then rejects for having no tax location. A customer
     * always exists by the time this runs, since the address is only editable
     * once subscribe has made one, so no customer is an error rather than
     * something to create.
     *
    ...
    public function handle(User $user, array $address): ServiceResult
    {
         ...
         * Stripe only touches what it is sent and an absent key leaves the old
         * value in place. An empty string is what clears one.
         */
        $address = [
            'city' => $address['city'] ?? '',
