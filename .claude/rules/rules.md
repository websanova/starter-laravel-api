...
- One rules class per resource (`UserRules`, `BookmarkRules`, `TagRules`, `SubscriptionRules`) plus `SharedRules` for cross-resource fields (`perPage`, `sortDir`, `search`, `trashed`, `id`, `token`, `verificationCode`).
