...
- Account store requests for gated resources check plan feature limits in `authorize()` via `$this->user()->canUsePlanFeature()`, aborting 403 with `responses.plan.limit_reached` when exceeded.
