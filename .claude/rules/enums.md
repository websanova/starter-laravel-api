...
- Sort columns per resource: `UserSort`, `BookmarkSort`, `TagSort`.
...
- Subscription: `SubscriptionMode` (`freemium`, `trial`, `required`), `PlanTier` (`Free`, `Pro`), `PlanInterval` (`Monthly`, `Yearly`), `PlanFeature` (`Bookmarks`, `Tags` with `relation()` and `isCountable()` methods), `PlanSort` (`Name`, `Tier`, `CreatedAt`).
