...
- Subscription: `SubscriptionMode` (`freemium`, `trial`, `required`), `PlanTier` (`Free`, `Pro`), `PlanInterval` (`Monthly`, `Yearly`), `PlanFeature` (`Bookmarks`, `Categories`, `Tags` with `relation()` and `isCountable()` methods), `PlanSort` (`Name`, `Tier`, `CreatedAt`).
