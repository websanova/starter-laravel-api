...
- `subscribed` (`EnsureSubscribed`) - enforces subscription access based on `config('subscription.mode')`: freemium always allows, trial requires complimentary plan or trial or active subscription, required requires complimentary plan or active subscription. Returns 403 when denied.
