...
- `subscribed` (`EnsureSubscribed`) - enforces subscription access based on `config('subscription.mode')`: freemium always allows, trial requires an active trial or subscription, required requires an active subscription. Returns 403 when denied.
- Stack order on `/account`: `auth:sanctum`, `track-active`, then `verified` and `password-updated` on inner routes. Gated resources (bookmarks, tags) add `subscribed` middleware.
