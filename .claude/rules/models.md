...
- `Price` model belongs to `Plan`, one row per `PlanInterval` (monthly, yearly). `amount` stored in cents, plus `stripe_product_id` and `stripe_price_id`. `Price::sync()` retrieves the Stripe product's default price and updates `stripe_price_id` and `amount`. `Plan::sync(?PlanInterval)` loops the plan's priced rows and delegates to each `Price::sync()`, optionally limited to one interval.
